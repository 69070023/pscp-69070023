...
```text
โจทย์ให้เราคำนวณราคาค่าจัดส่งพัสดุตามต้นทาง ปลายทาง และน้ำหนักของพัสดุ

- Input: รับชื่อเมืองต้นทาง ชื่อเมืองปลายทาง และน้ำหนักของพัสดุ
- Output: แสดงตัวเลขราคาค่าจัดส่งพัสดุ แต่ถ้าเส้นทางหรือข้อมูลไม่อยู่ในเงื่อนไข ให้แสดงคำว่า Error

เงื่อนไขสำคัญคือ ต้องเช็คว่าต้นทางกับปลายทางไม่ใช่เมืองเดียวกัน ข้อมูลรูปแบบถูกต้อง และน้ำหนักต้องไม่ติดลบหรือไม่อยู่ในเงื่อนไขตาราง
```
...
```text
Step 1: รับค่าต้นทาง ปลายทาง และน้ำหนัก
Step 2: เช็คว่าเส้นทางต้นทางและปลายทางเข้ากรณีไหนในตาราง
Step 3: คำนวณราคาตามสูตรของแต่ละกรณี
Step 4: ถ้าไม่มีเส้นทางในตาราง หรือข้อมูลไม่ถูกต้อง ให้สั่งแสดงผล Error
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกเลยครับ แค่ตอนแรกเขียนเช็คเงื่อนไขน้ำหนักผิดไปนิดหน่อย วิธีที่ใช้คือการเช็คเส้นทางที่ตรงกัน นำค่าน้ำหนักเข้าไปคำนวณตามเงื่อนไขของเคสนั้นๆ แล้วพ่นราคาออกมา ถ้าไม่ตรงเคสไหนเลยก็ให้พ่น Error
```
...
```text
ทดสอบกรณีที่ต้นทางกับปลายทางเป็นเมืองเดียวกัน (ชื่อซ้ำกัน)
```
...
```text
ทดสอบกรณีที่กรอกชื่อเมืองไม่อยู่ในระบบหรือไม่ตรงตามฟอร์แมต
```
...
```text
ทดสอบกรณีใส่น้ำหนักติดลบ เพื่อดูว่าโปรแกรมดักจับคำนวณผิดพลาดได้ไหม
```
