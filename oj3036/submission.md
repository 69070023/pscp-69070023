...
```text
โจทย์นี้ให้รับหมายเลขห้องมา 1 ห้อง แล้วหาว่าต้องทุบกำแพงอย่างน้อยกี่ครั้งเพื่อเดินทางจากห้องนั้นไปถึงห้องหมายเลข 1 โดยห้องต่าง ๆ จะถูกจัดเรียงเป็นชั้นตามรูปแบบที่โจทย์กำหนด

Input:
รับจำนวนเต็ม 1 จำนวน คือหมายเลขห้องที่ต้องการเริ่มต้น

Output:
แสดงจำนวนครั้งที่น้อยที่สุดที่ต้องทุบกำแพงเพื่อไปถึงห้องหมายเลข 1

Constraints:
ต้องหาวิธีที่ใช้จำนวนครั้งในการทุบกำแพงให้น้อยที่สุด และถ้าเริ่มต้นที่ห้องหมายเลข 1 อยู่แล้ว คำตอบจะเป็น 0 เพราะไม่ต้องทุบกำแพงเลย
```
...
```text
ตอนแรกผมสังเกตว่าห้องแต่ละชั้นมีรูปแบบของเลขที่เป็นกำลังสอง เลยคิดว่าจะหาก่อนว่าห้องที่รับมาอยู่ชั้นไหน โดยใช้ while loop ไล่หาชั้น

พอรู้ชั้นแล้วก็หาตำแหน่งของห้องในชั้นนั้น จากนั้นค่อยไล่ย้อนลงมาทีละชั้น พร้อมนับจำนวนครั้งที่ต้องทุบกำแพง โดยดูว่าตำแหน่งของห้องเป็นเลขคู่หรือเลขคี่ แล้วปรับตำแหน่งกับจำนวนครั้งที่ทุบไปเรื่อย ๆ จนลงมาถึงชั้นล่างสุด
```
...
```text
วิธีสุดท้ายค่อนข้างเหมือนกับแผนแรก คือผมหาชั้นของห้องก่อนโดยใช้ while loop แล้วหาตำแหน่งของห้องในชั้นนั้น

จากนั้นผมใช้ loop ไล่ลงมาทีละชั้น โดยดูว่าตำแหน่งปัจจุบันเป็นเลขคู่หรือเลขคี่ ถ้าเป็นเลขคู่จะเพิ่มจำนวนการทุบกำแพง 1 ครั้ง แต่ถ้าเป็นเลขคี่จะเพิ่ม 2 ครั้ง แล้วปรับตำแหน่งกับชั้นไปเรื่อย ๆ จนถึงชั้นล่างสุด สุดท้ายก็แสดงจำนวนครั้งที่ต้องทุบกำแพงทั้งหมด
```
...
```text
เลือกทดสอบห้องหมายเลข 1 เพราะเป็นห้องปลายทางอยู่แล้ว จึงไม่ควรต้องทุบกำแพงเลย
```
...
```text
เลือกเลข 50 เพื่อทดสอบว่าการหาชั้น ตำแหน่งของห้อง และการนับจำนวนกำแพงทำงานได้ถูกต้องกับค่าทั่วไป
```
