...
```text
โจทย์ให้เรานับจำนวนตัวเลขในช่วงตั้งแต่ A ถึง B ว่ามีกี่ตัวที่ตรงตามเงื่อนไขเศษการหารที่กำหนด

- Input: รับค่าจำนวนเต็ม 4 ตัว ได้แก่ ตัวเลขเริ่มต้น (A), ตัวเลขสิ้นสุด (B), ตัวหาร, และเศษที่ต้องการ
- Output: พ่นจำนวนตัวเลขทั้งหมดในช่วงตั้งแต่ A ถึง B ที่หารแล้วได้เศษตรงตามที่กำหนด

เงื่อนไขสำคัญคือต้องวนลูปเช็คตัวเลขทีละตัวให้ครบช่วงตั้งแต่น้อยไปมาก และเช็คเศษให้ตรงกับเงื่อนไข
```
...
```text
1. รับค่าตัวแปร A, B, ตัวหาร, และเศษ
2. วนลูปเช็คตัวเลขทีละตัวตั้งแต่น้อยไปมาก (จาก A ถึง B)
3. เอาตัวเลขแต่ละตัวมา % (mod) ด้วยตัวหาร ดูว่าได้เศษตรงกับที่โจทย์ต้องการไหม
4. ถ้าเศษตรงเงื่อนไข ให้บวกตัวนับสะสมเพิ่มขึ้น 1
5. พิมพ์ตัวนับสะสมออกมาเป็นคำตอบ
```
...
```text
วิธีสุดท้ายเหมือนกับแผนแรกครับ เพราะลองใช้วิธีวนลูปเช็คทีละตัวตามปกติแล้ว โปรแกรมทำงานได้เร็วพอ ไม่ติด Time Limit และให้ผลลัพธ์ถูกต้องตรงตามโจทย์ทุกกรณี
```
...
```text
ทดสอบกรณีตัวอย่างแรกที่มีช่วงตัวเลขแคบๆ (10 ถึง 20) เพื่อเช็คตรรกะการวนลูปและการเช็คเศษเบื้องต้น
```
...
```text
ทดสอบช่วงตัวเลขที่กว้างขึ้น (30 ถึง 100) เพื่อดูว่าการวนลูปนับข้ามช่วงยาวขึ้นยังได้ค่าถูกต้องไหม
```
...
```text
ทดสอบช่วงตัวเลขหลักพัน เพื่อดูว่าถ้าค่าเริ่มต้นและตัวหารมีขนาดใหญ่ขึ้น โปรแกรมยังทำงานได้ปกติ
```
