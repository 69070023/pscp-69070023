...
```text
โจทย์นี้ให้รับจำนวนเต็มมา 1 จำนวน แล้วหาจำนวนที่หาร 10 ลงตัวและมีค่าไม่เกินจำนวนที่รับมา จากนั้นให้แสดงตัวเลขลดลงทีละ 10 จนถึง 0
Input:
รับจำนวนเต็ม 1 จำนวน
Output:
แสดงตัวเลขที่หาร 10 ลงตัว เริ่มจากค่าที่ใกล้กับ input มากที่สุด แล้วลดลงทีละ 10 จนถึง 0 โดยแต่ละตัวคั่นด้วยช่องว่าง
Constraints:
ถ้าค่าที่รับมาหาร 10 ไม่ลงตัว ต้องตัดเศษออกก่อน เช่น รับ 67 จะเริ่มแสดงจาก 60 แล้วลดลงเป็น 50, 40, 30 ไปเรื่อย ๆ จนถึง 0
```
...
```text
ตอนแรกผมคิดว่าจะรับจำนวนเต็มเข้ามาก่อน แล้วหาค่าที่หาร 10 ลงตัวและใกล้กับเลขที่รับมามากที่สุด โดยใช้เศษจากการ mod 10 มาลบออก

หลังจากนั้นก็ใช้ loop แสดงค่าทีละตัว แล้วลดลงครั้งละ 10 ไปเรื่อย ๆ จนถึง 0
```
...
```text
วิธีสุดท้ายที่ใช้ก็เหมือนกับแผนแรก คือรับจำนวนเต็มเข้ามา แล้วใช้ number % 10 เพื่อหาเศษและลบเศษออก ทำให้ได้ค่าที่หาร 10 ลงตัว

จากนั้นใช้ while loop แสดงตัวเลขออกมาทีละค่า โดยลดลงครั้งละ 10 จนถึง 0 แล้วจบการทำงาน
```
...
```text
เลือกเพื่อเช็กว่าโปรแกรมสามารถตัดเศษออก แล้วเริ่มจากเลขที่หาร 10 ลงตัวได้ถูกต้อง
```
...
```text
เลือกเพื่อเช็กว่าถ้า input หาร 10 ลงตัวอยู่แล้ว โปรแกรมจะเริ่มแสดงจากเลขนั้นได้เลย
```
...
```text
เลือกเพื่อทดสอบกรณีค่าค่อนข้างเล็ก และดูว่าหลังตัดเศษแล้วโปรแกรมจะได้ 0 และจบได้ถูกต้อง
```
