...
```text
โจทย์นี้ให้คำนวณว่าแต่ละบ้านจะถูกน้ำหมึกท่วมในวินาทีที่เท่าไร
Input:
รับค่า s ซึ่งเป็นอัตราการเพิ่มพื้นที่ของน้ำหมึก และ n คือจำนวนบ้านที่ต้องการตรวจสอบ
จากนั้นรับพิกัด x และ y ของบ้านแต่ละหลังทั้งหมด n หลัง
Output:
แสดงเวลาที่น้ำหมึกจะไปถึงบ้านแต่ละหลังเป็นจำนวนวินาที โดยถ้าค่าที่คำนวณได้เป็นทศนิยมต้องปัดขึ้นเป็นจำนวนเต็ม เช่น 1.3 วินาที ต้องแสดงผลเป็น 2 วินาที
Constraints:
ต้องคำนวณตามพิกัดของบ้านแต่ละหลัง และเวลาที่ได้ต้องปัดขึ้นเป็นวินาทีเต็ม
```
...
```text
ตอนแรกผมคิดว่าจะรับค่า s กับ n มาก่อน แล้วใช้ loop รับค่า x, y ของบ้านตามจำนวน n หลัง จากนั้นค่อยหาว่าบ้านแต่ละหลังอยู่ไกลจากจุดเริ่มต้นของน้ำหมึกแค่ไหน แล้วนำไปคำนวณว่าใช้เวลากี่วินาที สุดท้ายก็ปัดค่าขึ้นแล้วแสดงผลออกมา
ตอนนั้นผมพอเข้าใจขั้นตอนคร่าวๆ แต่ยังไม่แน่ใจว่าต้องใช้สูตรอะไรในการคำนวณเวลา
```
...
```text
วิธีสุดท้ายก็คล้ายกับแผนแรก คือรับค่า s กับ n แล้ววนรับพิกัด x, y ของบ้านแต่ละหลัง จากนั้นใช้สูตรคำนวณเวลาที่น้ำหมึกจะไปถึงบ้าน แล้วปัดค่าขึ้นเป็นวินาทีเต็ม
...
```text
เลือกกรณีนี้เพราะมีทั้งบ้านที่อยู่ตรงจุดเริ่มต้น (0,0) และบ้านที่อยู่ไกลออกไป เพื่อดูว่าโปรแกรมคำนวณได้ถูกต้องทั้งกรณีพิเศษและกรณีปกติ
```
...
```text
เลือกเปลี่ยนค่า s จำนวนบ้าน และพิกัด เพื่อทดสอบว่าโปรแกรมยังคำนวณได้ถูกต้องเมื่อ input ไม่เหมือนกับ case แรก
```
...
```text
เลือกกรณีที่ไม่มีบ้านอยู่ตรงจุด (0,0) และใช้พิกัดที่มีค่ามากขึ้น เพื่อเช็กว่าการคำนวณระยะและการปัดเวลาเป็นวินาทีเต็มยังทำงานถูกต้อง
```
