...
```text
โจทย์ข้อนี้ให้เขียนโปรแกรมจำลองการผสมแม่สีโดยมีแม่สีหลักแค่ 3 สีเท่านั้นที่ระบบยอมรับคือแดง เหลือง น้ำเงิน ถ้าเอามาผสมกันจะได้สีตามนี้
Red + Yellow จะได้ Orange
Red + Blue จะได้ Violet 
Yellow + Blue จะได้ Green
แต่ถ้ามีการพิมพ์สีอื่นที่นอกเหนือจาก 3 แม่สีนี้เข้ามา โปรแกรมต้องฟ้องว่า Error ทันที
Input: รับค่าชื่อสีเข้ามา 2 บรรทัด
Output: เป็นชื่อสีใหม่ที่ผสมได้ หรือคำว่า Error ถ้ามีสีแปลกปลอมหลุดเข้ามา
```
...
```text
Step 1: อ่านข้อความชื่อสีบรรทัดที่ 1
Step 2: อ่านข้อความชื่อสีบรรทัดที่ 2
Step 3: ตรวจสอบเงื่อนไขว่า 2 สีที่รับมาตรงกับคู่ผสมแม่สี
Step 4: ตรวจสอบเงื่อนไขการสลับที่สีและกรณีสีเดียวกันทั้งคู่
Step 5: ถ้าตรงกับเงื่อนไขใด ให้พิมพ์ชื่อสีผลลัพธ์นั้นออกมา
Step 6: ถ้าไม่ตรงกับเงื่อนไขใดเลย ให้พิมพ์ "Error"
```
...
```text
เหมือนแผนแรกครับ วิธีที่ใช้จริงคือการใช้เงื่อนไข if-elif-else ตรวจสอบคู่สีให้ครบทุกกรณี เหตุผลที่ใช้วิธีนี้และไม่เปลี่ยนแผนเพราะเงื่อนไขของโจทย์มีแม่สีแค่ 3 สี ทำให้การจับคู่ผสมสีมีจำนวนกรณีที่เกิดขึ้นได้น้อยมาก การเขียนดักเงื่อนไขไล่ไปทีละบรรทัดจึงเป็นวิธีที่เรียบง่ายที่สุด
```
...
```text
เพื่อทดสอบเงื่อนไข if-elif-else
```
...
```text
เพื่อทดสอบการดักจับข้อผิดพลาดหรือทดสอบ else
```
